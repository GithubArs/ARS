...
import zipfile, string, random, os, json, shutil
...
def timelog(request):
    ...
    return render(request, 'gallery/timelog.html', {'time':time})

def user_timelog(request):
    time = Time.objects.all()
    return render(request, 'gallery/timelog.html', {'time':time})

def login(request):
    return render(request, 'gallery/account.html')


def admin_acc(request):
    return render(request, 'gallery/admin.html')
